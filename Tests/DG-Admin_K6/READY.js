...
export default function () {
  orderCodes.forEach(orderCode => {
    const url = `https://platform-dev.delivergate.com/api/v1/ready-to-pickup/${orderCode}`;
    const payload = '["ready"]';
    const headers = {
    ...
    check(res, {
      'Order Ready - status 200': (r) => r.status === 200,
    });
    if (res.status === 200) {
      console.log(`Order ${orderCode} ready successfully.`);
    } else {
      console.error(`Failed to ready order ${orderCode}. Status: ${res.status} Body: ${res.body}`);
    }
  });
}
